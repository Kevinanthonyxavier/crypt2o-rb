...
import { db } from "@/lib/firebase" // Adjust this import based on your project's structure

// interface CountdownProps {
//   // Remove targetDate prop as it will be fetched from Firestore
// }
...
export function CountdownTimer() {
  ...
  const [targetDate, setTargetDate] = useState<Date | null>(null); // State to hold the target date

  useEffect(() => {
    const fetchTargetDate = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "adminsettings")); // Replace with your collection name
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          // Assuming the target date is stored as a timestamp in Firestore
          if (data.targetDate) {
            setTargetDate(new Date(data.targetDate.seconds * 1000)); // Convert Firestore timestamp to Date
          }
        });
      } catch (error) {
  ...
  useEffect(() => {
    if (!targetDate) return; // Exit if targetDate is not set
